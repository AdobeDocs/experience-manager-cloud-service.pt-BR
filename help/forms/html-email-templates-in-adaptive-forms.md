---
title: Modelos de email do HTML no Adaptive Forms no Forms as a Cloud Service
...
1. Selecione ou faça upload de um modelo do HTML ou insira manualmente o modelo de email.
1. Teste a configuração do email.

## Configurar um modelo de HTML para um formulário adaptável

Você pode configurar um Formulário adaptável para enviar um email ao enviar usando a ação de envio [**Enviar email**](/help/forms/configure-submit-action-send-email.md). A ação fornece dois métodos para configurar um template do HTML:

### Opção 1: selecione um arquivo contendo o modelo HTML

Antes de continuar, verifique se você carregou o modelo do HTML no ambiente do AEM Forms.
...
1. Clique no ícone de pasta da opção Caminho do modelo externo e procure para selecionar seu modelo do HTML.
1. Clique em **Concluído** para salvar a configuração.

O modelo do HTML agora está configurado para o Formulário adaptável.

### Opção 2: inserir ou colar manualmente um modelo do HTML

1. Abra o Formulário adaptável para edição.
1. Vá para o **Navegador de Conteúdo**, selecione o **Contêiner do Guia** e toque no ícone de propriedades. Uma caixa de diálogo com o título `Adaptive Form Container` é exibida.
1. Vá para a guia **Envio** e selecione a ação de envio **Enviar email**.
1. Habilite a opção **Usar modelo de HTML**.
1. Digite ou cole seu código do HTML diretamente na caixa **Modelo de email** fornecida.
...
### Modelo de email do HTML de amostra

Este é um exemplo de um modelo de email do HTML que usa espaços reservados para dados de formulário:
...
* Verifique se há tags ausentes ou incorretas no modelo do HTML.
