...
## Adição de filtros e regras de cache no Dispatcher para implantação as a Cloud Service do Screens {#deployment}
...
* Adicionar o seguinte à seção `/invalidate` em `publish_farm.any`.
...
* Adicione as seguintes regras à seção `/rules` em `/cache` em publish_farm.any ou em um arquivo incluído em `publish_farm.any`.
