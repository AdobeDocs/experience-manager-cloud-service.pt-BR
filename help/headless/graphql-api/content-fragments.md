...
>[!NOTE]
>
>* Por padrão, a paginação usa a UUID do nó do repositório que representa o fragmento para fazer a ordem dos resultados sempre ser a mesma. Quando `sort` é usado, o UUID será usado implicitamente para garantir uma ordem de classificação exclusiva, mesmo para dois itens com chaves de classificação idênticas.
>
...
* usar `_dynamicUrl` na referência `ImageRef`

* adicionar `_assetTransform` ao cabeçalho de lista no qual seus filtros são definidos
...
### Limitações da entrega de imagem {#image-delivery-limitations}
...
* Além dos campos do modelo, há alguns campos gerados pelo sistema (precedidos por um sublinhado):
   ...
   * Para [entrega de imagem](#image-delivery):
      ...
      * `_dynamicUrl`: na referência `ImageRef`

      * `_assetTransform`: no cabeçalho de lista no qual os filtros são definidos
