...
| Gerar variações predefinidas de ativos carregados automaticamente | Processar automaticamente ativos aproveitando a tecnologia de manipulação e transformação de mídia exclusiva do Adobe para ações predefinidas. Crie uma lógica personalizada para definir suas próprias ações usando APIs e microsserviços de ativos. | Interface do [!DNL Assets] no  |
