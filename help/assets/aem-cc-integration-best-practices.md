...
| Gerar variações predefinidas de ativos carregados automaticamente | Processar automaticamente ativos aproveitando a tecnologia de manipulação e transformação de mídia exclusiva do Adobe para ações predefinidas. Crie uma lógica personalizada para definir suas próprias ações usando APIs e microsserviços de ativos. | [!DNL Assets] interface do usuário |
...
* [Experience Manager Assets Brand ](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/home.html) Portal para um complemento configurável do Assets para publicar ativos.

* As soluções personalizadas são criadas com base no código [Asset Share Commons](https://opensource.adobe.com/asset-share-commons/) base.
* Experience Manager [compartilhamento de link](/help/assets/share-assets.md) para compartilhar ativos ad hoc usando links.
* [A interface da Web do Assets ](/help/assets/manage-digital-assets.md) com áreas para terceiros externos garantidas pela configuração do Controle de acesso do Experience Manager e pelos ajustes necessários de configuração de TI/rede, dando a esses usuários externos acesso ao Experience Manager.
...
### Acesso aos ativos da Adobe Stock do Experience Manager Assets {#access-to-adobe-stock-assets-from-aem-assets}

[A ](/help/assets/aem-assets-adobe-stock.md) integração do Experience Manager e Adobe Stock fornece aos usuários do Experience Manager a capacidade de pesquisar, visualizar, licenciar e salvar ativos do Adobe Stock no Experience Manager. Os ativos licenciados e salvos do Adobe Stock selecionaram metadados de Estoque, que podem ser usados para pesquisá-los com filtros extras.

Alguns pontos importantes sobre essa integração:

* Quando os ativos do Adobe stock são salvos no Experience Manager, eles se tornam Experience Manager Assets comuns, com o binário salvo no repositório do Experience Manager. Alguns metadados relacionados ao Adobe Stock são salvos para o ativo no Experience Manager, caso contrário, o processo de assimilação será igual ao de qualquer outro arquivo. Por exemplo, se as Tags inteligentes estiverem ativas, as tags serão adicionadas a esses ativos ao salvar.
