...
description: Publish ou cancelar a publicação de ativos no Experience Manager Assets, Dynamic Media e Brand Portal
mini-toc-levels: 1
...
A Publicação rápida permite publicar imediatamente o conteúdo no destino selecionado. No console [!DNL Experience Manager Assets], navegue até a pasta principal e selecione todos os ativos ou pastas que deseja publicar. Clique na opção **[!UICONTROL Publish rápido]** na barra de ferramentas e selecione o destino na lista suspensa onde deseja publicar os ativos.
...
* [!UICONTROL Ações]
   * `Publish`: ativos e pastas do Publish para o destino selecionado
   * `Unpublish`: cancelar a publicação de ativos e pastas do destino

* [!UICONTROL Destino]
   * `Publish`: ativos e pastas do Publish para [!DNL Experience Manager Assets] (`AEM`)
   * `Dynamic Media`: Publish Assets para [!DNL Dynamic Media]
   * `Brand Portal`: ativos e pastas do Publish para [!DNL Brand Portal]

* [!UICONTROL Programação]
   * `Now`: ativos do Publish imediatamente
   * `Later`: ativos do Publish com base na data ou hora `Activation`
...
Depois de aplicar os filtros, clique em **[!UICONTROL OK]** e em **[!UICONTROL Publish]**. Ao clicar no botão Publicar, uma mensagem de confirmação `Resource(s) have been scheduled for publication` é exibida. E os ativos e (ou) pastas selecionados são publicados no destino definido com base no agendador (`Now` ou `Later`). Faça logon na instância de publicação para verificar se os ativos e (ou) as pastas foram publicados com êxito.

![Publish para AEM](assets/manage-publication-publish-aem.png)

Na ilustração acima, você pode ver valores diferentes para o atributo **[!UICONTROL Publish Target]**. Lembre-se de que você optou por publicar em [!DNL Experience Manager Assets] (`Destination: Publish`). Então, por que ele mostra que apenas uma pasta e um ativo são publicados em `AEM`, e os outros dois ativos são publicados em `AEM` e `Dynamic Media`?
...
A tabela a seguir explica como ocorre a publicação dependendo do **[!UICONTROL Destino]** e do **[!UICONTROL modo Dynamic Media Publish]** definidos:

| [!UICONTROL Destino] | [!UICONTROL Modo Dynamic Media Publish] | [!UICONTROL Publish Target] | Conteúdo permitido |
| --- | --- | --- | --- |
...
>
>Se você selecionar uma pasta (`Selective Publish`) e escolher o destino [!DNL Dynamic Media], o atributo [!UICONTROL Publish Target] refletirá `None`.
...
![Publish para Dynamic Media](assets/manage-publication-dynamic-media.png)

>[!NOTE]
>
>Se [!DNL Dynamic Media] não estiver configurado na instância [!DNL Experience Manager Assets] e o **[!UICONTROL Destino]** for **[!UICONTROL Publish]**, os ativos e as pastas serão sempre publicados em `AEM`.
>
...
1. Clique em **[!UICONTROL Publish]** em **[!UICONTROL Ação]** e selecione o **[!UICONTROL Destino]** onde deseja publicar o conteúdo.
...
Somente os ativos são publicados em [!DNL Dynamic Media]. No entanto, o comportamento de publicação é diferente com base nas propriedades da pasta. Uma pasta pode ter o **[!UICONTROL modo Dynamic Media Publish]** configurado para publicação seletiva, que pode ser qualquer um dos seguintes:

* `Selective Publish`
* `Immediate`
* `Upon Activation`

O processo de publicação para **[!UICONTROL Imediato]** e **[!UICONTROL Na Ativação]** é consistente, no entanto, diferente para **[!UICONTROL Publish Seletiva]**. Consulte [configurar publicação seletiva no nível da pasta no Dynamic Media](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/dynamicmedia/selective-publishing.html?lang=pt-BR). Depois de configurar a publicação seletiva em uma pasta, siga um destes procedimentos:
...
* [Cancelar a publicação de ativos de forma seletiva do Dynamic Media ou do Experience Manager usando Gerenciar publicação](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/dynamicmedia/selective-publishing.html?lang=en#selective-unpublish-manage-publication)
* [Publish Assets para Dynamic Media ou Experience Manager usando o Quick Publish](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/dynamicmedia/selective-publishing.html?lang=en#quick-publish-aem-dm)
