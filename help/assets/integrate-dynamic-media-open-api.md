---
title: Integrar o Supervisor de conteúdo à API aberta do Dynamic Media
description: Integrar o Content Advisor a vários aplicativos da Adobe, que não sejam da Adobe e de terceiros.
role: Admin, User
...
# Integração do Dynamic Media com recursos OpenAPI {#integrate-dynamic-media-open-apis}

O Supervisor de conteúdo permite a integração usando vários aplicativos da Adobe para que eles trabalhem em conjunto sem problemas.

## Pré-requisitos {#prereqs-polaris}

Use os seguintes pré-requisitos se estiver integrando o Supervisor de Conteúdo com os recursos do Dynamic Media com OpenAPI:

* [Métodos de comunicação](/help/assets/content-advisor-properties.md#prereqs)
...
A integração do Supervisor de conteúdo com o processo OpenAPI do Dynamic Media envolve várias etapas que incluem a criação de um URL de mídia dinâmica personalizado ou pronto para escolher o URL de mídia dinâmica etc.

### Integrar o Content Advisor for Dynamic Media aos recursos da OpenAPI {#integrate-dynamic-media}
...
Essa configuração permite visualizar todos os ativos aprovados sem pastas ou como uma estrutura simples. Para obter mais informações, navegue até a propriedade `aemTierType` em [Propriedades do Supervisor de Conteúdo](/help/assets/content-advisor-properties.md).


### Criar um URL de entrega dinâmico a partir de ativos aprovados {#create-dynamic-media-url}

Depois de configurar o Supervisor de conteúdo, um esquema de objetos será usado para criar um URL de entrega dinâmico dos ativos selecionados.
...
Por exemplo, no Supervisor de Conteúdo para Dynamic Media com recursos OpenAPI, uma propriedade em `asset jcr:content/metadata/client_name:market` é convertida em `metadata.embedded.client_name:market` para configuração de filtro.
