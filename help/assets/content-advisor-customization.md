---
title: Personalizar Supervisor de Conteúdo
description: Use funções para personalizar o Supervisor de Conteúdo no aplicativo.
role: Admin, User
...
# Personalizações do Supervisor de conteúdo {#content-advisor-customization}

O Supervisor de Conteúdo permite personalizar vários componentes de acordo com preferências, requisitos ou necessidades funcionais. Você pode personalizar os seguintes componentes [Supervisor de Conteúdo de Microfront-end](#overview-asset-selector.md):
...
O Supervisor de conteúdo permite controlar o uso de um ativo expirado. Você pode personalizar o ativo expirado com um selo **Expirando em breve** que pode ajudá-lo a saber com antecedência sobre os ativos que expirarão em 30 dias a partir da data atual. Além disso, isso pode ser personalizado de acordo com a exigência. Você também pode permitir a seleção de um ativo expirado na tela ou vice-versa. A personalização de um ativo expirado pode ser feita usando alguns snippets de código de várias maneiras:
...
Você pode personalizar o uso de um ativo expirado para torná-lo selecionável ou não selecionável. Você pode personalizar se deseja permitir ou não o arrastar e soltar de um ativo expirado na tela Supervisor de conteúdo. Para fazer isso, use os seguintes parâmetros para tornar um ativo não selecionável na tela:
...
## Fazer upload no Supervisor de conteúdo {#upload-in-content-advisor}

Você pode fazer upload de arquivos ou pastas para o Supervisor de Conteúdo a partir do sistema de arquivos local. Para fazer upload de arquivos usando o sistema de arquivos local, geralmente é necessário usar um recurso de upload fornecido por um aplicativo de micro front-end do Supervisor de conteúdo. Os `upload` Vários trechos de código necessários para invocar o upload no Supervisor de Conteúdo envolvem:
...
*Mais propriedades incluem `metadataSchema`, `onMetadataFormChange`, `targetUploadPath`, `hideUploadButton`, `onUploadStart`, `importSettings` `onUploadComplete`, `onFilesChange`,`uploadingPlaceholder`*. Consulte [Propriedades do Supervisor de Conteúdo](/help/assets/content-advisor-properties.md) para obter mais informações.
