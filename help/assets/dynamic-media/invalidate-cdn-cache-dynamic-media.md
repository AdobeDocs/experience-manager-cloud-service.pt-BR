...
Os ativos da Dynamic Media são armazenados em cache pela CDN (Content Delivery Network, Rede de conteúdo) para obter delivery rápido aos seus clientes. No entanto, quando você faz atualizações nesses ativos, deseja que essas alterações entrem em vigor imediatamente em seu site. Limpar ou invalidar o cache CDN permite atualizar rapidamente os ativos entregues pela Dynamic Media. Não é mais necessário aguardar a expiração do cache usando um valor TTL (Time To Live) (o padrão é dez horas). Em vez disso, você pode enviar uma solicitação na interface do usuário do Dynamic Media para que o cache expire em minutos.
...
| *Alguns URLs não estão definidos corretamente. Corrija e envie novamente.* | Ocorre se a API de invalidação do cache CDN IPS retornar um erro. O erro indica que o URL se refere a uma empresa diferente ou que o URL não é válido de acordo com a validação realizada pela API cdnCacheInvalidation IPS. |
