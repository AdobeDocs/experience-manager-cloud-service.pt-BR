...
1. Clique em **[!UICONTROL Pré-visualização]** ![pré-visualização](assets/preview.png) para gerar uma pré-visualização do painel Filtros como aparece depois de adicionar o predicado.
...
1. Para fechar a pré-visualização, clique em **[!UICONTROL Close]** ![close](assets/do-not-localize/close_icon.png) no canto superior direito da pré-visualização.
...
O predicado `Multi Value Property` permite que você pesquise ativos para obter vários valores. Considere um cenário em que você tem imagens de vários produtos no AEM Assets e os metadados de cada imagem incluem um número SKU associado ao produto. Você pode usar esse predicado para procurar imagens de produtos com base em vários números de SKU.
...
O predicado `Tags` permite que você realize pesquisas baseadas em tags para ativos. Por padrão, a AEM Assets pesquisa ativos por uma ou mais tags correspondentes com base nas tags especificadas. Em outras palavras, o query de pesquisa executa uma operação OU usando as tags especificadas. No entanto, você pode usar a opção de correspondência de todas as tags para pesquisar ativos que incluem todas as tags especificadas.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Predicado de pesquisa para executar pesquisa de texto completo em um nó de ativo inteiro. Ele é mapeado com o operador <code>jcr</code>:<code>contains</code>. Você pode especificar um caminho relativo se desejar executar uma pesquisa de texto completo em uma parte específica do nó do ativo.</td>
   <td>
    <ul>
     <li>Etiqueta</li>
...
1. Toque em **[!UICONTROL Excluir]** ![ícone de exclusão](assets/do-not-localize/deleteoutline.png) na barra de ferramentas.
