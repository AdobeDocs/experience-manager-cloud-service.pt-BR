...
1. A mesma caixa de diálogo é aberta. Você pode inserir um **Rótulo** e uma **Comentário**, se necessário.
1. Confirme a nova versão com a opção **Criar**.
...
* a opção **Restaurar** na parte superior da [barra de ferramentas Ações](/help/sites-cloud/authoring/getting-started/basic-handling.md#actions-toolbar)
   ...
   * **Restaurar versão**
     ...
     Restaure as versões das páginas especificadas na pasta selecionada no momento. Também pode incluir a restauração de páginas que foram excluídas anteriormente.

   * **Restaurar árvore**

     Restaure uma versão de uma árvore inteira em uma data e hora especificadas. Também pode incluir páginas que foram excluídas anteriormente.
...
>1. Restaure a primeira versão; ou seja, 1.0.
...
Este método pode ser usado para restaurar versões de páginas especificadas na pasta atual. Também pode incluir a restauração de páginas que foram excluídas anteriormente:
...
Esse método pode ser usado para restaurar uma versão de uma árvore, como em uma data e hora especificadas. Pode incluir páginas que foram excluídas anteriormente:
...
1. Usar o seletor de data e hora em **Versões mais recentes à data** para que você possa selecionar outra versão da árvore, ou seja, a que será restaurada.
...
1. Selecionar **Comparar ao atual**. A variável [diff de página](/help/sites-cloud/authoring/features/page-diff.md) abre e exibe as diferenças.
...
* Significa que a versão mostrada foi criada/ativada *antes* o ponto no tempo selecionado no Timewarp.
* Ao navegar para uma página que foi excluída, ela também é renderizada - desde que as versões antigas da página ainda estejam disponíveis no repositório.
* Se nenhuma versão publicada for encontrada, o Timewarp reverterá para o estado atual da página no ambiente de criação (o motivo é evitar um erro de página/404, que impediria a navegação).
...
1. Na caixa de diálogo, defina uma data e hora de destino e clique em **Definir data**. Se você não selecionar uma hora, o padrão será usar a hora atual.

   ![Data de destino do Timewarp](/help/sites-cloud/authoring/assets/versions-timewarp-target.png)

1. A página é exibida com base no conjunto de datas. O modo Timewarp é indicado por meio da barra de status azul na parte superior da janela. Use os links na barra de status para poder selecionar uma nova data de destino ou sair do modo Timewarp.

   ![No modo Timewarp](/help/sites-cloud/authoring/assets/versions-timewarp.png)

### Limitações do Timewarp {#timewarp-limitations}

O Timewarp se esforça ao máximo para reproduzir uma página em um ponto selecionado no tempo. No entanto, devido às complexidades da criação contínua de conteúdo no AEM, essa reprodução nem sempre é possível. Lembre-se dessas limitações ao usar o Timewarp.
...
* **O Timewarp é somente leitura** - não é possível editar a versão antiga da página. Ela só está disponível para exibição. Se quiser restaurar a versão mais antiga, faça isso manualmente usando [restaurar](#revert-to-a-version).
* **O Timewarp é baseado no conteúdo da página** - Se os elementos para renderizar o site forem alterados, como código, css e ativos, a exibição será diferente da original. Esses itens não têm controle de versão no repositório.
