...
* The [configuration](/help/sites-cloud/authoring/fundamentals/editing-content.md#component-toolbar) dialog for specific components (using the ? ícone na barra de ferramentas da caixa de diálogo), que mostra a ajuda sensível ao contexto
...
Os componentes são os elementos básicos do conteúdo do AEM. Você coloca vários componentes em uma página e configura suas opções para criar sua página de conteúdo com o AEM.
...
The components browser is a tab within the side panel (together with the [assets browser](#assets-browser) and [content tree](#content-tree)). Para abrir (ou fechar) o painel lateral, use o ícone na parte superior esquerda da barra de ferramentas:
...
The assets browser is a tab within the side panel along with the [components browser](#components-browser) and [content tree](#content-tree). Para abrir ou fechar o painel lateral, use o ícone na parte superior esquerda da barra de ferramentas:
...
Uma página pode ser facilmente composta de vários componentes do mesmo tipo, de modo que a árvore de conteúdo (componente) exibe o texto descritivo (em cinza) após o nome do tipo de componente (em preto). O texto descritivo é originado de propriedades comuns do componente, como título ou texto.
...
![Expansão da divisa da árvore de conteúdo](/help/sites-cloud/authoring/assets/content-tree-chevron.png)
...
* Por exemplo, uma página básica:
   ...
   ![Árvore de conteúdo destacada](/help/sites-cloud/authoring/assets/content-tree-highlighted.png)

   Os componentes de uma página básica terão as opções habituais.
   ...
   ![Botão de edição da Árvore de conteúdo](/help/sites-cloud/authoring/assets/content-tree-edit.png)

* Uma página que faz parte de uma live-copy, na qual os componentes são herdados de outra página terá uma seleção reduzida de opções, incluindo as opções de herança. <!--A page that is part of a [livecopy](/help/sites-administering/msm.md), where components are inherited from another page:-->
...
* [Inicializações](/help/sites-cloud/authoring/launches/overview.md), fornece acesso a inicializações relacionadas
...
Para os recursos adequados (por exemplo, as páginas no console **Sites** ou os ativos no console **Assets**), a [linha do tempo pode ser usada para mostrar a atividade recente de qualquer item selecionado](/help/sites-cloud/authoring/getting-started/basic-handling.md#timeline).
...
* [Promover lançamento](/help/sites-cloud/authoring/launches/promoting.md) (somente se a página for uma inicialização)
...
* [Layout](/help/sites-cloud/authoring/features/responsive-layout.md) - permite criar e editar seu layout responsivo dependendo do dispositivo (se a página for baseada em um contêiner de layout)
...
Os campos de caminho têm funcionalidade de preenchimento automático e de antecipação agora para facilitar a localização de um recurso.
...
O navegador de caminho é organizado da mesma maneira que a [exibição de colunas](/help/sites-cloud/authoring/getting-started/basic-handling.md#column-view) do console de sites, permitindo uma seleção mais detalhada dos recursos.
