...
* Fotos ilustrativas:
São imagens de pessoas, objetos ou lugares. Think about the role of the photo in the page; an appropriate text equivalent is likely to be `Photo of [object]`, but may be dependent on the surrounding text.
* Ícones:
...
* **Utilização de cabeçalhos:** Desde que você tenha os recursos de acessibilidade do RTE ativados, o AEM oferece três níveis de cabeçalho de página. Você pode usá-los para identificar as seções e subseções de conteúdo. O Cabeçalho 1 é o de nível mais alto e o Cabeçalho 3 o de nível mais baixo. O administrador do sistema pode configurar o sistema para permitir o uso de mais níveis de cabeçalho.
* **Texto** destacado: Use o elemento `<strong>` ou `<em>` para indicar ênfase. Não use os cabeçalhos para destacar o texto dentro dos parágrafos.
...
* **Use listas**: você pode usar o HTML para especificar três diferentes tipos de listas:
   ...
   * The `<ol>` element is used for *numbered* lists. Individual list items are identified using the `<li>` element. No RTE, use o ícone **Lista numerada** .
...
* **Usar tabelas**: Tabelas de dados devem ser identificadas usando elementos de tabela HTML:
   * one `<table>` element
...
* **Tabelas** de dados complexos: Em alguns casos, quando há tabelas complexas com dois ou mais níveis de cabeçalhos, as Propriedades básicas da tabela podem não ser suficientes para fornecer todas as informações estruturais necessárias. For these kinds of complex tables, direct relationships need to be created between the headers and their related cells using the **header** and **id** attributes. Por exemplo, na tabela abaixo os cabeçalhos e ids são combinados para fazer uma associação programática para os usuários de tecnologias de assistência.
...
Se a cor for usada como uma dica para fornecer informações, você deverá fornecer uma dica visual adicional, como a alteração do estilo (por exemplo, negrito, itálico) ou da fonte. Isso ajuda as pessoas com pouca visão ou daltonismo a identificar a informação. No entanto, isso não pode ser totalmente confiável, pois não ajudará pessoas que não conseguem ver a página.
...
Se as imagens de texto tiverem que ser usadas, use o CSS para substituir as imagens de texto por texto equivalente em HTML, de modo que o texto esteja disponível de forma personalizável. Para ver um exemplo, consulte o [C30: usar o CSS para substituir o texto por imagens de texto e fornecer controles de interface do usuário para fazer a alteração](https://www.w3.org/TR/2008/NOTE-WCAG20-TECHS-20081211/C30).
