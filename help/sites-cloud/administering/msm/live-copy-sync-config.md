...
>[!CAUTION]
>
>Atualizar ou alterar uma configuração de implementação pronta para uso é **not** recomendado. Se houver um requisito para uma ação ativa personalizada, ela deverá ser adicionada em uma configuração de implementação personalizada.
...
* **Na implantação**: O **Implantação** é usado na página de impressão azul ou na variável **Sincronizar** é usado na página Live Copy.
...
>[!NOTE]
>
>Utilização do **Na modificação** pode afetar o desempenho. Consulte [Práticas recomendadas do MSM](best-practices.md#onmodify) para obter mais informações.
...
|---|---|---|
| `contentCopy` | Quando os nós da origem não existem na Live Copy, essa ação copia os nós para a Live Copy. [Configure o **Ação de cópia de conteúdo MSM CQ** service](#excluding-properties-and-node-types-from-synchronization) para especificar os tipos de nó, itens de parágrafo e propriedades de página a serem excluídos. |  |
| `contentDelete` | Essa ação exclui os nós da Live Copy que não existem na origem. [Configure o **Ação de exclusão de conteúdo MSM CQ** service](#excluding-properties-and-node-types-from-synchronization) para especificar os tipos de nó, itens de parágrafo e propriedades de página a serem excluídos. |  |
| `contentUpdate` | Esta ação atualiza o conteúdo da Live Copy com as alterações da origem. [Configure o **Ação de atualização de conteúdo do MSM CQ** service](#excluding-properties-and-node-types-from-synchronization) para especificar os tipos de nó, itens de parágrafo e propriedades de página a serem excluídos. |  |
| `editProperties` | Essa ação edita as propriedades da Live Copy. O `editMap` determina quais propriedades são editadas e seu valor. O valor da variável `editMap` deve usar o seguinte formato:<br>`[property_name_n]#[current_value]#[new_value]`<br>`current_value` e `new_value` são expressões regulares e `n` é um número inteiro incrementado.<br>Por exemplo, considere o seguinte valor como `editMap`:<br>`sling:resourceType#/(contentpage`líqua`homepage)#/mobilecontentpage,cq:template#/contentpage#/mobilecontentpage`<br>Esse valor edita as propriedades dos nós da Live Copy da seguinte maneira:<br>O `sling:resourceType` propriedades definidas como `contentpage` ou `homepage` estão definidas como `mobilecontentpage`.<br>O `cq:template` propriedades definidas como `contentpage` estão definidas como `mobilecontentpage`. | `editMap: (String)` identifica a propriedade, o valor atual e o novo valor. Consulte a descrição para obter mais informações. |
...
| `referencesUpdate` | Esta ação de sincronização atualiza referências na Live Copy.<br>Ele pesquisa caminhos nas páginas da Live Copy que apontam para um recurso dentro do blueprint. Quando encontrado, ele atualiza o caminho para apontar para o recurso relacionado dentro da Live Copy. As referências que têm destinos fora do blueprint não são alteradas. <br>[Configure o **Ação de atualização de referências do MSM CQ** service](#excluding-properties-and-node-types-from-synchronization) para especificar os tipos de nó, itens de parágrafo e propriedades de página a serem excluídos. |  |
...
| `mandatoryStructure` | Esta ação define a permissão do `ActionSet.ACTION_NAME_REMOVE` ACL na página Live Copy como somente leitura para um grupo de usuários específico.<br>Use esta ação somente para páginas. | `target: (String)` é a ID do grupo para o qual você está definindo permissões. |
...
| Tipos de nó Mixin ignorados | `cq.wcm.msm.action.ignoredMixin` | Uma expressão regular que corresponde aos nomes dos tipos de nó mixin a serem excluídos da ação de sincronização (disponível somente para `contentUpdate` action) |
...
* **[Propriedades da página da Live Copy](live-copy-sync-config.md#setting-the-rollout-configurations-for-a-live-copy-page):** Quando uma página de Live Copy é configurada para usar uma ou mais configurações de implementação, o MSM usa essas configurações de implementação.
...
* **[Padrão do sistema](live-copy-sync-config.md#setting-the-system-default-rollout-configuration):** Quando a configuração de implementação da página principal da Live Copy não pode ser determinada, a configuração de implementação padrão do sistema é usada.

Por exemplo, um blueprint usa o [Tutorial WKND](/help/implementing/developing/introduction/develop-wknd-tutorial.md) site como conteúdo de origem. Um site é criado a partir do blueprint. Cada item da lista a seguir descreve um cenário diferente sobre o uso de configurações de implementação:
...
Você também pode configurar as configurações de implementação para uma página de Live Copy ao [criar a Live Copy](creating-live-copies.md#creating-a-live-copy-of-a-page).
...
1. Use o **Sites** para selecionar a página raiz do blueprint.
1. Selecione **Propriedades** na barra de ferramentas.
...
* **Gerente de relacionamento dinâmico do WCM CQ do dia** com o PID de serviço `com.day.cq.wcm.msm.impl.LiveRelationshipManagerImpl`

Configure o serviço usando a variável [console da web](/help/implementing/deploying/configuring-osgi.md#osgi-configuration-with-the-web-console) ou [nó do repositório](/help/implementing/deploying/configuring-osgi.md#osgi-configuration-in-the-repository).
...
Defina esse valor de propriedade como o caminho da configuração de implementação a ser usada como padrão do sistema. O valor padrão é `/libs/msm/wcm/rolloutconfigs/default`, que é o **Configuração de implementação padrão**.
