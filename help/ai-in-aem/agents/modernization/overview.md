...
# Visão geral do Agente de modernização de experiência {#experience-modernization-agent}

Saiba como o Agente de modernização de experiência integra sites no Edge Delivery Services com a ajuda da IA.
...
O Agente de modernização de experiência desbloqueia o valor total do Edge Delivery Services (incluindo a criação no AEM) tornando as migrações de site e a evolução contínua rápidas e sem atritos.
...
## Recursos de desenvolvimento de blocos {#block-development}

O Agente de modernização de experiência aproveita os recursos gerais de desenvolvimento do Edge Delivery Services que atendem a várias tarefas de desenvolvimento, fornecendo valor contínuo além da criação ou migração inicial do site.

* Segue a metodologia de CDD (Content-Driven Development) para modelos de conteúdo amigáveis
* Aproveita a [Coleção de Blocos](https://www.aem.live/developer/block-collection) e a [Parte do Bloco](https://www.aem.live/developer/block-party/) para encontrar implementações de referência e práticas recomendadas
* Oferece suporte a fluxos de trabalho de teste e depuração para validação de alterações antes da implantação

Os recursos detalhados incluem desenvolvimento de blocos, modelagem de conteúdo, detecção de blocos de referência, testes e depuração.
...
* Conteúdo dinâmico complexo, como conteúdo que requer uma interação sofisticada do usuário para aparecer no DOM.
   * O conteúdo renderizado do lado do cliente é compatível se o conteúdo for acessível por meio de um URL específico.
   * Elementos ocultos por meio do CSS, mas presentes no DOM como guias, acordeões ou carrosséis também são compatíveis.

O agente não dá suporte aos seguintes destinos.

* Ambientes de publicação do AEM em que os sites usam entrega baseada em HTL
...
* Integrações de dados/serviços de terceiros do servidor ou do lado do cliente
* Integrações do comércio ou da funcionalidade de pesquisa
* Camada de dados da MarTech ou direcionamento/experimentação
* Isolamento de fragmentos de conteúdo/experiência
* Herança multissite (MSM)
* Funcionalidade personalizada (por exemplo, calculadoras, configuradores)
