...
>[!NOTE]
>Para obter mais informações sobre o Dispatcher na nuvem e como baixar as Ferramentas do Dispatcher, consulte o [Dispatcher na nuvem](/help/implementing/dispatcher/disp-overview.md) página. Se a configuração do dispatcher estiver no modo herdado, consulte o [documentação do modo herdado](/help/implementing/dispatcher/validation-debug-legacy.md).
...
Este artigo pressupõe que a configuração do dispatcher do seu projeto inclua o arquivo `opt-in/USE_SOURCES_DIRECTLY`, que faz com que o SDK e o tempo de execução validem e implantem a configuração de maneira aprimorada em comparação ao modo herdado, removendo limitações sobre o número e o tamanho dos arquivos.

Dessa forma, se a configuração do dispatcher não incluir o arquivo mencionado acima, será **altamente recomendado** que você migre do modo herdado para o modo flexível, conforme descrito na seção [Migração do modo herdado para o modo flexível](#migrating) seção.
...
Há quatro seções na configuração do farm, nas quais você pode incluir seus próprios arquivos: `/clientheaders`, `filters`, `/rules` in `/cache` seção e `/virtualhosts`. Os arquivos incluídos precisam ser nomeados da seguinte maneira:
...
No modo flexível, outros arquivos também podem ser incluídos, desde que estejam localizados em subdiretórios (em qualquer nível) de `conf.d` diretório prefixado da seguinte maneira.
...
>Para Windows e macOS, recomendamos o uso do Docker Desktop.
...
Seus arquivos locais imutáveis podem ser atualizados executando o `bin/update_maven.sh src/dispatcher` script na pasta do dispatcher, onde `src/dispatcher` é o diretório de configuração do dispatcher. Isso também atualizará qualquer arquivo pom.xml no diretório pai para que as verificações de imutabilidade do Maven também sejam atualizadas.

## Depuração da configuração do Apache e Dispatcher {#debugging-apache-and-dispatcher-configuration}

Observe que é possível executar o Apache Dispatcher localmente usando `./bin/docker_run.sh src/dispatcher docker.for.mac.localhost:4503 8080`.
...
Observe que as primeiras linhas de saída serão semelhantes ao que seria executado para `docker_run.sh`, por exemplo:
...
1. **Teste local.** Usando o SDK de ferramentas mais recentes do Dispatcher, adicione a pasta e o arquivo `opt-in/USE_SOURCES_DIRECTLY`. Siga as instruções de &quot;validação local&quot; neste artigo para testar se o dispatcher funciona localmente.
