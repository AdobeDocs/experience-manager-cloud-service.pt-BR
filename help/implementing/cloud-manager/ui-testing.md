...
O AEM fornece um conjunto integrado de [quality gates (portais de qualidade) do Cloud Manager](/help/implementing/cloud-manager/custom-code-quality-rules.md) para garantir atualizações tranquilas para aplicativos personalizados. Em particular, os portais de teste de TI já permitem a criação e a automação de testes personalizados usando APIs AEM.

Os testes de interface do usuário são testes baseados em Selenium, compactados em uma imagem do Docker, para permitir uma variedade de opções de linguagens e estruturas (como Java e Maven, Node e WebDriver.io, ou qualquer outra estrutura e tecnologia criada no Selenium). Além disso, um projeto de testes de interface do usuário pode ser facilmente gerado usando [o Arquétipo do Projeto AEM.](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=pt-BR)
...
>
>O Adobe também fornece um exemplo de módulo de teste de IU com base em Java e WebDriver. Consulte a [Repositório de amostras de teste do AEM](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-selenium-webdriver) para obter detalhes.
...
1. Decida sobre a linguagem de programação que deseja usar.
   ...
   * Para JavaScript e WDIO, use o código de amostra gerado automaticamente no `ui.tests` pasta do seu repositório do Cloud Manager.

      >[!NOTE]
      >
      >Se o repositório foi criado antes da criação automática do Cloud Manager `it.tests` pastas, você também poderá gerar a versão mais recente usando o [Arquétipo de projeto AEM.](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/it.tests)

   * Para Java e WebDriver, use o código de amostra do [Repositório de amostras de teste do AEM.](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-selenium-webdriver)
...
1. Garantir que o teste da interface do usuário esteja ativado de acordo com a seção [Aceitação do cliente](#customer-opt-in) neste documento.

1. Desenvolver os casos de teste e [execute-os testes localmente.](#run-ui-tests-locally)
...
* O arquivo deve estar sob o submódulo maven para testes de interface do usuário ao lado do `pom.xml` arquivo do submódulo de testes de interface.
...
* Para a variável baseada em JavaScript, `ui.tests` pasta gerada com base no [Arquétipo de projeto AEM](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/ui.tests), você pode executar o comando abaixo para adicionar a configuração necessária.
...
* As amostras de teste Java fornecidas já têm o sinalizador de aceitação definido.
...
| `AEM_PUBLISH_USERNAME` | `admin` | O nome de usuário para fazer logon na instância de publicação AEM |
...
Os exemplos de teste de Adobe fornecem funções auxiliares para acessar os parâmetros de configuração:

* JavaScript: consulte o [lib/config.js](https://github.com/adobe/aem-project-archetype/blob/develop/src/main/archetype/ui.tests/test-module/lib/config.js) módulo
* Java: consulte [Configuração](https://github.com/adobe/aem-test-samples/blob/aem-cloud/ui-selenium-webdriver/test-module/src/main/java/com/adobe/cq/cloud/testing/ui/java/ui/tests/lib/Config.java) classe
...
As amostras de teste da interface do Adobe lidam com isso com o script `wait-for-grid.sh`, que é executado na inicialização do Docker e inicia a execução de teste real somente depois que a grade está pronta.
...
>[!NOTE]
>
>O resultado da etapa de teste da interface do usuário é avaliado somente com base nos relatórios de teste. Gere o relatório adequadamente para a execução do teste.
>
>Use asserções em vez de apenas registrar um erro em STDERR ou retornar um código de saída diferente de zero, caso contrário, o pipeline de implantação poderá continuar normalmente.
...
Você pode usar as funções auxiliares para criar capturas de tela por meio dos testes.
...
Se um arquivo de resultados de teste for criado durante uma execução de teste da interface, você poderá baixá-lo do Cloud Manager usando o `Download Details` sob o botão [**Testes de interface do usuário personalizados** etapa.](/help/implementing/cloud-manager/deploy-code.md)
...
1. Faça upload do arquivo na URL especificado pela variável de ambiente `UPLOAD_URL`.
   ...
   * Os exemplos de teste de Adobe fornecem funções auxiliares para o upload de arquivos:
      * JavaScript: consulte o [getFileHandleForUpload](https://github.com/adobe/aem-project-archetype/blob/develop/src/main/archetype/ui.tests/test-module/lib/wdio.commands.js) comando.
      * Java: consulte [ManipuladorArquivo](https://github.com/adobe/aem-test-samples/blob/aem-cloud/ui-selenium-webdriver/test-module/src/main/java/com/adobe/cq/cloud/testing/ui/java/ui/tests/lib/FileHandler.java) classe.
...
## Execução de testes de interface do usuário localmente {#run-ui-tests-locally}

Antes de ativar testes de interface do usuário em um pipeline do Cloud Manager, é recomendável executar os testes de interface do usuário localmente em direção à [SDK AS A CLOUD SERVICE AEM](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md) ou em uma instância as a Cloud Service real do AEM.

### Pré-requisitos {#prerequisites}

Os testes no Cloud Manager serão executados usando um usuário administrador técnico.

Para executar os testes de interface do usuário no computador local, crie um usuário com permissões de administrador para obter o mesmo comportamento.

### Amostra de teste do JavaScript {#javascript-sample}

1. Abra um shell e navegue até o `ui.tests` pasta no seu repositório
...
>* Você precisa garantir que está na versão mais recente do Chrome, pois o teste baixa a versão mais recente do ChromeDriver automaticamente para teste.
>
>Para obter detalhes, consulte o [Repositório do Arquétipo de Projeto AEM.](https://github.com/adobe/aem-project-archetype/blob/develop/src/main/archetype/ui.tests/README.md)

### Amostra de teste do Java {#java-sample}

1. Abra um shell e navegue até o `ui.tests/test-module` pasta no seu repositório
