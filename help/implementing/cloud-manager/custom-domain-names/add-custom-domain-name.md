...
1. Na caixa de diálogo **Verificar domínio**, com base no tipo de certificado selecionado, siga um destes procedimentos:
   ...
   >[!NOTE]
   >
   >Se você usa seu próprio certificado SSL gerenciado pelo cliente (OV/EV ou DV) e pretende usar um ***provedor*** de CDN gerenciado pelo cliente, ignore a adição de um certificado SSL e vá diretamente para [Adicionar uma configuração de CDN](/help/implementing/cloud-manager/cdn-configurations/add-cdn-config.md) quando estiver pronto.
