...
* A Auditoria de conteúdo é um recurso ativado nos pipelines de produção do Sites do Cloud Manager. A configuração do pipeline de produção para programas com Sites agora inclui uma terceira guia chamada **Auditoria de conteúdo**. Sempre que um pipeline de produção é executado, uma nova etapa de auditoria de conteúdo é incluída nele após o teste funcional personalizado que avaliará o site em relação a várias dimensões, incluindo desempenho, SEO (otimização de mecanismos de pesquisa), acessibilidade, práticas recomendadas e PWA (aplicativo da Web progressivo).
