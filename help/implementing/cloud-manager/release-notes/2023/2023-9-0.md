...
Interessado em testar o novo painel? Comece enviando um email para `aem-lighthouse-pilot@adobe.com` do email associado à Adobe ID.

## Correções de erros {#bug-fixes}

* Quando um programa é excluído, qualquer pipeline associado em execução agora também é excluído.
* Se um pipeline estiver em andamento, a variável **Enviar** botão do **Ativação concluída** agora a caixa de diálogo está desativada e informa ao usuário que a data de ativação não pode ser definida devido à execução do pipeline.
