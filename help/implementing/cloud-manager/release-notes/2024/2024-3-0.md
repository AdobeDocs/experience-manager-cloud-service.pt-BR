...
* [Agora você pode criar uma infraestrutura de rede avançada](/help/security/configuring-advanced-networking.md) em seu programa Cloud Manager e configurá-la em seus ambientes de maneira automatizada usando a interface do usuário do Cloud Manager.
* [Os detalhes da etapa de execução do pipeline](/help/implementing/cloud-manager/configuring-pipelines/managing-pipelines.md#view-details) incluirão qual é a etapa atual da implantação e o que se espera vir a seguir.
...
* Um erro foi corrigido quando um usuário define a variável `COMMERCE_ENDPOINT` com um espaço à direita e o Dispatcher falha ao carregar.
