...
As solicitações de conteúdo são solicitações que entram no AEM Sites (incluindo em conexão com o Edge Delivery Services para AEM Sites) ou em qualquer sistema de cache fornecido pelo cliente (como uma Rede de entrega de conteúdo) para fornecer conteúdo ou dados no formato HTML por meio de exibições de página (por exemplo, páginas e fragmentos de experiência) ou no formato JSON por meio de chamadas de API (de forma headless). As solicitações de conteúdo são contadas como uma exibição de página ou cinco chamadas de API e são medidas na entrada do primeiro sistema de cache a receber uma solicitação de conteúdo. Certas solicitações HTTP são incluídas ou excluídas para fins de contagem de solicitações de conteúdo. A lista completa dessas solicitações HTTP incluídas e excluídas, bem como suas definições técnicas, estão disponíveis na documentação.

## Noções básicas sobre solicitações de conteúdo Cloud Service {#understanding-cloud-service-content-requests}

Para clientes que usam o CDN pronto para uso, as solicitações de conteúdo de Cloud Service são medidas por meio da coleção de dados do lado do servidor. Essa coleção é habilitada por meio da análise de log da CDN. As solicitações de conteúdo são coletadas automaticamente no lado do servidor, na borda do Adobe Experience Manager as a Cloud Service, por meio da análise automatizada dos arquivos de log originados da CDN as a Cloud Service AEM. Isso é feito isolando as solicitações que retornam o HTML `(text/html)` ou JSON `(application/json)` conteúdo da CDN e se baseia em várias regras de inclusão e exclusão detalhadas abaixo. Uma solicitação de conteúdo ocorre independentemente do conteúdo retornado que está sendo veiculado a partir dos caches CDN ou do conteúdo que retorna à origem do CDN (despachantes AEM).

Para clientes que utilizam sua própria CDN, a coleção do lado do cliente oferece um reflexo mais preciso das interações, garantindo uma medida confiável do envolvimento do site por meio do [Monitoramento de uso real](/help/sites-cloud/administering/real-use-monitoring-for-aem-as-a-cloud-service.md) serviço. Isso fornece aos clientes insights avançados sobre o tráfego e o desempenho da página. Embora seja benéfico para todos os clientes, oferece uma reflexão representativa das interações do usuário, garantindo uma medida confiável de envolvimento do site capturando o número de exibições de página do lado do cliente.

Para clientes que acrescentam sua própria CDN ao AEM as a Cloud Service, os relatórios do lado do servidor resultam em números que não podem ser usados para comparação com as solicitações de conteúdo licenciadas. Com o [Monitoramento de uso real](/help/sites-cloud/administering/real-use-monitoring-for-aem-as-a-cloud-service.md), o Adobe pode refletir uma medida confiável de engajamento no site.


### Variações de solicitações de conteúdo Cloud Service {#content-requests-variances}

As solicitações de conteúdo podem ter variações nas ferramentas de relatório de Analytics de uma organização, conforme resumido na tabela a seguir. Em geral, *não* use ferramentas do analytics que reúnem dados por meio de instrumentação do lado do cliente para relatar o número de solicitações de conteúdo para um determinado site, simplesmente porque elas geralmente dependem do consentimento do usuário para serem acionadas, perdendo uma fração significativa do tráfego. As ferramentas do Analytics que reúnem dados do lado do servidor em arquivos de log ou relatórios CDN para clientes que adicionam seu próprio CDN além do AEM as a Cloud Service fornecerão contagens melhores. Para a criação de relatórios sobre exibições de página e o desempenho associado, a variável [Serviço de Dados Adobe RUM](/help/sites-cloud/administering/real-use-monitoring-for-aem-as-a-cloud-service.md) é a opção Adobe recomendada.
...
| Monitoramento as a Cloud Service do New Relic com AEM | Excluído | Monitoramento global as a Cloud Service do AEM. |
...
| Serviço de aquecimento de pod as a Cloud Service AEM | Excluído |
