...
1. logForwarding.yaml deve conter metadados e uma configuração semelhante ao seguinte formato (usamos o Splunk como exemplo).
   ...
   O nó padrão deve ser incluído por motivos de compatibilidade futura.

   O parâmetro kind deve ser definido como LogForwarding, a versão deve ser definida como a versão do schema, que é 1.
...
* Autentique usando o token SAS, que deve ter um período de validação mínimo.
* O token SAS deve ser criado na página da conta, não na página do container.
