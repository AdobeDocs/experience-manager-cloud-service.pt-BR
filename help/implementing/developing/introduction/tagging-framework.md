...
* O nó de conteúdo marcado `NodeType` deve incluir a [`cq:Taggable`](#taggable-content-cq-taggable-mixin) combinação.
* O objeto [`TagID`](#tagid) é adicionado à [`cq:tags`](#cq-tags-property) propriedade do nó de conteúdo e é resolvido em um nó do tipo [`cq:Tag`.](#cq-tag-node-type)

## cq:Tipo de nó de tag {#cq-tag-node-type}

A declaração de uma tag é capturada no repositório em um nó do tipo `cq:Tag.`

* Uma tag pode ser uma palavra simples (por exemplo, `sky`) ou representar uma taxonomia hierárquica (por exemplo, `fruit/apple`, quer dizer, quer se trate do fruto genérico quer da maçã mais específica).
* As tags são identificadas por uma única `TagID`.
...
* A `jcr:title` propriedade (o Título a ser exibido na interface do usuário) é opcional.
...
Geralmente, o `TagID` é um redutor `TagID` que começa com a namespace ou pode ser um absoluto `TagID` a partir do nó raiz de [taxonomia.](#taxonomy-root-node)

Quando o conteúdo é marcado, se ainda não existir, a [`cq:tags`](#cq-tags-property) propriedade é adicionada ao nó de conteúdo e a propriedade `TagID` é adicionada ao valor de `String` matriz da propriedade.

A `TagID` consiste em uma [namespace](#tag-namespace) seguida pelo local `TagID`. [As tags](#container-tags) de container têm subtags que representam uma ordem hierárquica na taxonomia. As subtags podem ser usadas para referenciar tags como qualquer local `TagID`. Por exemplo, marcar conteúdo com `fruit` é permitido, mesmo se for uma tag de container com subtags, como `fruit/apple` e `fruit/banana`.
...
Todas as tags têm uma namespace. Se nenhuma namespace for especificada, a tag será atribuída à namespace padrão, que é `TagID``default`, ou seja, `/content/cq:tags/default`.  O padrão do Título é `Standard Tags`nesses casos.
...
Além disso, as tags de container (ou super tags) em uma taxonomia servem como subsoma de todas as subtags: por exemplo, o conteúdo marcado com `fruit/apple` também é considerado marcado com `fruit` tags, ou seja, procurar por conteúdo marcado com `fruit` o também encontraria o conteúdo marcado com `fruit/apple`.
...
As tags que fazem referência a caminhos ou caminhos não existentes que não apontam para um `cq:Tag` nó são consideradas inválidas e são ignoradas.

A tabela a seguir mostra algumas amostras `TagID`s, seus elementos e como as `TagID` são resolvidas para um caminho absoluto no repositório:
...
As tags existem como nós no repositório no nó raiz [taxonomia.](#taxonomy-root-node) Permitir ou negar que autores e visitantes do site criem tags em uma determinada namespace pode ser alcançado ao configurar ACLs apropriadas no repositório.
...
* Permitir que usuários/autores gravem acesso às namespaces nas quais as tags devem ser livremente definidas pelos usuários/autores (`add_node` em `/content/cq:tags/some_namespace`)
...
>[!NOTE]
>
>É recomendável ativar somente tags no nó de nível superior de um item de conteúdo agregado (ou em seu `jcr:content` nó). Os exemplos incluem:
>
>* Páginas (`cq:Page`) em que o `jcr:content`nó é do tipo `cq:PageContent`, que inclui a `cq:Taggable` mistura.
>* Ativos (`cq:Asset`) nos quais o `jcr:content/metadata` nó sempre tem a `cq:Taggable` mistura.


### Nó de notação de tipo (CND) {#node-type-notation-cnd}

As definições de Tipo de nó existem no repositório como arquivos CND. A notação CND é definida como parte da documentação do [JCR.](https://jackrabbit.apache.org/node-type-notation.html).
...
A `cq:tags` propriedade é um `String` storage usado para armazenar um ou mais `TagID`s quando são aplicados ao conteúdo por autores ou visitantes do site. A propriedade só tem significado quando adicionada a um nó que é definido com a [`cq:Taggable`](#taggable-content-cq-taggable-mixin) mistura.
...
* A tag A não é excluída e recebe uma `cq:movedTo` propriedade.
   ...
   * Um valor especial para a `cq:movedTo` propriedade é `nirvana`, que é aplicado quando a tag é excluída, mas não pode ser removida do repositório, pois há subtags com uma `cq:movedTo` que deve ser mantida.
...
* A tag B é criada (no caso de uma movimentação) e recebe uma `cq:backlinks` propriedade.
   ...
   * Isso é necessário principalmente para manter `cq:movedTo` as propriedades atualizadas quando a tag B também é movida/unida/excluída ou quando a tag B é ativada, nesse caso, todas as tags de backlinks também devem ser ativadas.
