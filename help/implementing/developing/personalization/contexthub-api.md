...
A API Javascript do ContextHub está disponível para seus scripts quando o componente [ContextHub foi adicionado à página](configuring-contexthub.md#adding-contexthub-to-a-page-component).
...
A tabela a seguir lista os eventos de nomes que ocorrem nas Lojas do ContextHub. Consulte também [ContextHub.Utils.Eventing](contexthub-api.md#contexthub-utils-eventing).
...
Um [`ContextHub.Utils.Eventing`](#contexthub-utils-eventing) objeto. Use esse objeto para funções de vínculo para armazenar eventos. Para obter informações sobre o valor padrão e a inicialização, consulte [`init(name,config)`](#init-name-config).
...
Um `ContextHub.Utils.Persistence` objeto. Para obter informações sobre o valor padrão e a inicialização, consulte [`init(name,config)`](#init-name-config).
...
* **`options`:** (Objeto) Um objeto opcional de opções transmitido para a função setItem. Para obter informações, consulte o `options` parâmetro de [`setItem(key,value,options)`](#setitem-key-value-options).
...
* **`config`:** Um objeto que contém propriedades de configuração:
   ...
   * `eventing`: O objeto [ContextHub.Utils.Eventing](#contexthub-utils-eventing) para esta loja. O valor padrão é usado pelo `ContextHub.eventing` objeto.
...
Uma loja que contém dados JSON. Os dados são recuperados de um serviço JSONP externo, ou opcionalmente de um serviço que retorna dados JSON. Especifique os detalhes do serviço usando a [`init`](#init-name-config) função ao criar uma instância dessa classe.
...
ContextHub.Store.JSONPStore estende o [ContextHub.Store.Core](#contexthub-store-core) e herda as funções dessa classe.
...
`ContextHub.Store.PersistedJSONPStore` estende o [ContextHub.Store.JSONPStore](#contexthub-store-jsonpstore) para herdar todas as funções dessa classe. No entanto, os dados recuperados do serviço JSONP são persistentes de acordo com a configuração de persistência do ContextHub. (Consulte Modos de [persistência:](configuring-contexthub.md#persistence-modes))

## ContextHub.Store.PersistedStore {#contexthub-store-persistedstore}

`ContextHub.Store.PersistedStore` estende o [ContextHub.Store.Core](#contexthub-store-core) para que ele herde todas as funções dessa classe. Os dados neste repositório são persistentes de acordo com a configuração da persistência do ContextHub.

## ContextHub.Store.SessionStore {#contexthub-store-sessionstore}

`ContextHub.Store.SessionStore` estende o [ContextHub.Store.Core](#contexthub-store-core) para que ele herde todas as funções dessa classe. Os dados neste armazenamento são persistentes usando a persistência na memória (objeto Javascript).
...
Registra um renderizador de módulo de interface com o ContextHub. Depois que o renderizador é registrado, ele pode ser usado para [criar módulos](configuring-contexthub.md#adding-a-ui-module)de interface do usuário. Use essa função quando estiver [se estendendo `ContextHub.UI.BaseModuleRenderer`](extending-contexthub.md#creating-contexthub-ui-module-types) para criar um renderizador personalizado do Módulo de interface do usuário.
...
* **`options`:** Um objeto que contém valores de propriedade para configurar os atributos do cookie. Consulte a [`setItem`](#setitem-key-value-options) função para obter informações. A `expires` propriedade não tem efeito.
...
Remove todos os cookies que correspondem a um determinado filtro. Os cookies são combinados usando a `getKeys` função e removidos usando a `removeItem` função.
...
Permite vincular e desvincular funções a eventos de armazenamento do ContextHub. Acesse `ContextHub.Utils.Eventing` objetos de uma loja usando a propriedade [eventing](#eventing) da loja.
...
* **`name`:** O [nome do evento](#contexthub-utils-eventing) para o qual você está desvinculando a função.
* **`selector`:** O seletor que identifica o vínculo. (Consulte o `selector` parâmetro para as funções [`on`](#on-name-handler-selector-triggerforpastevents) e [`once`](#once-name-handler-selector-triggerforpastevents) ).
...
* **`name`:** (String) O [nome do evento](#contexthub-utils-eventing) ao qual você está vinculando a função.
...
* **`storeType`:** (String) O nome do tipo de loja. Consulte o `storeType` parâmetro da [`ContextHub.Utils.storeCandidates.registerStoreCandidate`](#contexthub-utils-storecandidates) função.
...
* `storeType`: (String) O nome do candidato da loja. Consulte o `storeType` parâmetro da [`ContextHub.Utils.storeCandidates.registerStoreCandidate`](#registerstorecandidate-store-storetype-priority-applies) função.
...
Uma matriz de valores de string, em que cada string é o tipo de armazenamento com o qual um candidato de loja foi registrado. Consulte o `storeType` parâmetro da [`ContextHub.Utils.storeCandidates.registerStoreCandidate`](#contexthub-utils-storecandidates) função.
