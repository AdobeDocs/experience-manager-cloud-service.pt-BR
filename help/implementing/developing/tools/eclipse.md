...
* Baixe e instale o [Eclipse IDE para desenvolvedores](https://www.eclipse.org/downloads/packages/)Enterprise Java.
* Configure a instalação do eclipse para garantir que você tenha pelo menos 1 gigabyte de memória heap, editando o arquivo de `eclipse.ini` configuração conforme descrito nas Perguntas frequentes do [Eclipse](https://wiki.eclipse.org/FAQ_How_do_I_increase_the_heap_size_available_to_Eclipse).
...
1. Abra o site [AEM Developer Tools.](https://eclipse.adobe.com/aem/dev-tools/)
...
1. Em **Nome** , insira `AEM Developer Tools`.
1. Em **Local** , copie o URL de instalação.
1. Clique em **Adicionar**.
1. Verifique os plug-ins **AEM** e **Sling** .
1. Clique em **Avançar**.
1. Na janela **Install Details (Detalhes** da instalação), clique em **Next (Avançar)** novamente.
...
No Eclipse, uma Perspectiva determina as ações e visualizações disponíveis em uma janela e permite a interação orientada por tarefa com recursos no Eclipse. Para obter mais detalhes sobre o Perspective, consulte a documentação do [Eclipse.](https://help.eclipse.org)
...
1. Forneça os seguintes campos para o projeto de amostra:
   ...
   * **Package** (Pacote) - talvez seja necessário expandir as opções **Advanced** (Avançado) para definir esse valor.
...
1. Click **Finish**. A estrutura do projeto é criada.
   ...
   >[!NOTE]
   >
   >Em uma nova instalação (mais especificamente, quando as dependências do Mac nunca foram baixadas), você pode criar o projeto com erros. Nesse caso, siga o procedimento descrito em [Resolução de definição](#resolving-invalid-project-definition)de projeto inválida.
...
1. Siga as instruções para criar um projeto [de vários módulos de](#sample-multi-module-project) amostra e você terá os seguintes projetos criados para você, o que permitirá uma saudável separação de preocupações:
...
1. Substitua o conteúdo do seu `PROJECT.ui.apps` projeto pelas pastas `apps` e `etc` pastas do seu pacote:

   1. No painel do Project Explorer, desdobre `PROJECT.ui.apps` > `src` > `main` > `content` > `jcr_root` > `apps`.
   1. Clique com o botão direito do mouse na `apps` pasta e escolha **Mostrar em** > **System Explorer**.
   1. Exclua as pastas `apps` e `etc` pastas que devem ser exibidas e coloque aqui as pastas `apps` e `etc` pastas do seu pacote de conteúdo.
   1. No Eclipse, clique com o botão direito do mouse no `PROJECT.ui.apps` projeto e escolha **Atualizar**.

1. Em seguida, faça o mesmo para a pasta de conteúdo `PROJECT.ui.content` e substitua-a por uma de suas embalagens:

   1. No painel do Project Explorer, desdobre `PROJECT.ui.content` > `src` > `main` > `content` > `jcr_root` > `content`.
   1. Clique com o botão direito do mouse na pasta de conteúdo mais profunda e escolha **Mostrar em** -> **System Explorer**.
   1. Exclua a pasta de conteúdo que deve ser exibida e coloque aqui a pasta de conteúdo do seu pacote de conteúdo.
   1. No Eclipse, clique com o botão direito do mouse no `PROJECT.ui.content` projeto e escolha **Atualizar**.

1. Agora você precisa atualizar os `filter.xml` arquivos desses dois projetos para corresponder ao conteúdo do seu pacote de conteúdo. Para isso, abra o `META-INF/vault/filter.xml` arquivo do pacote de conteúdo em um editor de texto/código separado.

   * Este é um exemplo de como seu `filter.xml` arquivo pode parecer:
...
1. Quanto ao conteúdo do pacote que foi dividido em dois projetos, você também terá que dividir essas regras de filtragem em dois e atualizar os `filter.xml` arquivos dos dois projetos de acordo.

   1. No Eclipse, abra `PROJECT.ui.apps/src/main/content/META-INF/filter.xml`.
   1. Substitua o conteúdo do `<workspaceFilter>` elemento pelas regras do seu pacote que são start `/apps` e `/etc`
      * Por exemplo:
         ...
         ```
   1. Então abra `PROJECT.ui.content/src/main/content/META-INF/filter.xml`.
   1. Substitua as regras pelas regras do seu pacote com o start `/content`.
      * Por exemplo:
...
Se desejar recriar um pacote do seu projeto, clique com o botão direito do mouse no `PROJECT.ui.apps` ou `PROJECT.ui.content` e escolha **Executar como** -> Instalação **Maven**.
