...
# Plug-in de Maven do Pacote de Conteúdo do Adobe {#adobe-content-package-maven-plugin}
...
>[!NOTE]
>
>A criação do pacote agora é de propriedade do [Apache Jackrabbit FileVault Package Maven plugin](https://jackrabbit.apache.org/filevault-package-maven-plugin/). A implantação dos pacotes construídos no AEM é executada pelo plugin Adobe Content Package Maven, conforme descrito aqui.
...
Para permitir que o Maven baixe o plug-in, use o perfil fornecido na seção [Obtenção do plug-in do pacote de conteúdo ](#obtaining-the-content-package-maven-plugin) nesta página.
...
Metas que usam proxies para AEM usam a primeira configuração de proxy válida encontrada nas configurações de Maven. Se nenhuma configuração de proxy for encontrada, nenhum proxy será usado. Consulte o parâmetro `useProxy` na seção [Parâmetros Comuns](#common-parameters).
...
Todos os parâmetros para a meta de build são descritos na seção [Parâmetros Comuns](#common-parameters).
...
Todos os parâmetros da meta de ls estão descritos na seção [Parâmetros Comuns](#common-parameters).
...
Todos os parâmetros da meta de rm são descritos na seção [Parâmetros Comuns](#common-parameters).
...
Além dos parâmetros a seguir, consulte a descrição do parâmetro `name` na seção [Parâmetros Comuns](#common-parameters).
...
## Usando o Arquétipo de Projeto AEM para Gerar Projetos AEM {#using-archetypes}
