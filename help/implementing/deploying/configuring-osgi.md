...
Por exemplo, se AEM estiver usando o autor e o desenvolvimento de modos de execução, os nós de configuração em `/apps/example/config.author/` e `/apps/example/config.author.dev/` serão aplicados, enquanto os nós de configuração em `/apps/example/config.publish/` e `/apps/example/config.author.stage/` não serão aplicados.
...
Existem três variedades de valores de configuração de OSGi que podem ser usadas com AEM como Cloud Service.
...
1. **Valores** específicos do ambiente, que são valores que variam entre ambientes de desenvolvimento e, portanto, não podem ser direcionados com precisão pelo modo de execução (já que há um único  `dev` modo de execução no AEM como um Cloud Service). Por exemplo:
...
Sempre que definir um valor de configuração OSGi, comece com valores em linha, qualquer configuração secreta ou específica do ambiente é selecionada somente se necessário para o caso de uso.
...
Use apenas configurações específicas do ambiente (`$[env:ENV_VAR_NAME]`) para valores de configuração não secretos quando os valores variam em ambientes de desenvolvimento. Isso inclui instâncias de desenvolvimento local e qualquer AEM como ambientes de desenvolvimento de Cloud Service. Evite usar configurações não secretas específicas do ambiente para AEM como ambientes de Cloud Service Stage ou Production.
...
* Em vez disso, use os valores em linha padrão nas configurações OSGi para valores não secretos de Preparo e Produção.  Relacionado a isso, não é recomendável usar configurações específicas do ambiente para facilitar as alterações de configuração em tempo de execução nos ambientes de Preparo e Produção; essas alterações devem ser introduzidas por meio do gerenciamento do código-fonte.
...
AEM como um Cloud Service requer o uso de configurações específicas do ambiente (`$[secret:SECRET_VAR_NAME]`) para quaisquer valores de configuração OSGi secretos, como senhas, chaves de API privadas ou quaisquer outros valores que não possam ser armazenados no Git por motivos de segurança.

Use configurações secretas específicas do ambiente para armazenar o valor de segredos em todos os ambientes AEM como Cloud Service, incluindo Preparo e Produção.
...
Há duas maneiras de criar novas configurações OSGi, conforme descrito abaixo. A primeira abordagem é normalmente usada para configurar componentes OSGi personalizados que têm propriedades e valores OSGi bem conhecidos pelo desenvolvedor, e a última para componentes OSGi fornecidos por AEM.

### Gravando configurações do OSGi {#writing-osgi-configurations}

Os arquivos de configuração OSGi formatados em JSON podem ser gravados manualmente diretamente no projeto AEM. Essa é frequentemente a maneira mais rápida de criar configurações OSGi para componentes OSGi bem conhecidos e, especialmente, componentes OSGi personalizados que foram projetados e desenvolvidos pelo mesmo desenvolvedor definindo as configurações. Essa abordagem também pode ser utilizada para copiar/colar e atualizar configurações para o mesmo componente OSGi em várias pastas do modo de execução.

1. No IDE, abra o projeto `ui.apps`, localize ou crie a pasta de configuração (`/apps/.../config.<runmode>`) que direciona os modos de execução que a nova configuração OSGi deve afetar
...
Observe que os nomes de arquivos de fábrica de configuração do OSGi usam a convenção  `<PID>-<factory-name>.cfg.json` de nomenclatura
...
### Gerar configurações OSGi usando o Início rápido do SDK AEM {#generating-osgi-configuratuions-using-the-aem-sdk-quickstart}

O AEM Web Console do AEM SDK Quickstart Jar pode ser usado para configurar componentes OSGi e exportar configurações OSGi como JSON. Isso é útil para configurar componentes OSGi fornecidos AEM cujas propriedades OSGi e seus formatos de valor podem não ser bem entendidos pelo desenvolvedor que define as configurações OSGi no projeto AEM. Observe que usando a interface de configuração do Console da Web AEM grava arquivos `.cfg.json` no repositório, portanto, esteja ciente disso para evitar um comportamento inesperado potencial durante o desenvolvimento local, quando as configurações OSGi definidas pelo Projeto AEM podem diferir das configurações geradas.

1. Faça logon no console AEM Web do AEM SDK Quickstart Jar como o usuário administrador
1. Navegue até OSGi > Configuração
1. Localize o componente OSGi a ser configurado e toque no título para editar
...
1. Registrar a Identidade Persistente (PID) em local seguro, isso será usado posteriormente para gerar a configuração JSON do OSGi
1. Toque em Salvar
...
1. No IDE, abra o projeto `ui.apps`, localize ou crie a pasta de configuração (`/apps/.../config.<runmode>`) que direciona os modos de execução em que a nova configuração OSGi deve entrar em vigor.
...
Os valores para segredos são lidos de arquivos. Portanto, para cada espaço reservado usando um segredo, um arquivo de texto contendo o valor secreto precisa ser criado.

Por exemplo, se `$[secret:server_password]` for usado, um arquivo de texto chamado **server_password** precisará ser criado. Todos esses arquivos secretos precisam ser armazenados no mesmo diretório e a propriedade da estrutura `org.apache.felix.configadmin.plugin.interpolation.secretsdir` precisa ser configurada com esse diretório local.
...
* devem ser usados nomes de variáveis independentes. É recomendável usar um prefixo, como `author_<variablename>` e `publish_<variablename>`, em que os nomes das variáveis são os mesmos

### Exemplos de configuração {#configuration-examples}

Nos exemplos abaixo, considere que há 3 ambientes de desenvolvimento, além dos ambientes de preparo e produção.
...
A intenção é que o valor da propriedade OSGI `my_var1` seja diferente para estágio, produção e para cada um dos três ambientes de desenvolvimento. Assim, a API do Cloud Manager precisará ser chamada para definir o valor de `my_var1` para cada env de desenvolvimento.
...
A intenção é que o valor da propriedade OSGi `my_var1` seja o mesmo para estágio, produção e apenas um dos ambientes de desenvolvimento, mas que seja diferente para os outros dois ambientes de desenvolvimento. Nesse caso, a API do Cloud Manager precisará ser chamada para definir o valor de `my_var1` para cada um dos ambientes de desenvolvimento, incluindo para o ambiente de desenvolvimento que deve ter o mesmo valor que o estágio e a produção. Ele não herdará o valor definido na pasta **config**.
...
Chamar a API implantará as novas variáveis e valores em um ambiente do Cloud, de modo semelhante a um pipeline típico de implantação de código do cliente. Os serviços de criação e publicação serão reiniciados e farão referência aos novos valores, normalmente levando alguns minutos.
...
Observe que as variáveis padrão não são definidas por meio da API, mas na própria propriedade OSGi.
...
## Considerações de implantação para valores de configuração específicos do ambiente e do segredo {#deployment-considerations-for-secret-and-environment-specific-configuration-values}

Como os valores de configuração específicos do ambiente e do segredo estão fora do Git e, portanto, não fazem parte do AEM formal como um mecanismo de implantação do Cloud Service, o cliente deve gerenciar, governar e integrar o AEM como um processo de implantação do Cloud Service.

Como mencionado acima, chamar a API implantará as novas variáveis e valores em ambientes do Cloud, de forma semelhante a um pipeline de implantação de código do cliente típico. Os serviços de criação e publicação serão reiniciados e farão referência aos novos valores, normalmente levando alguns minutos. Observe que as portas de qualidade e os testes que são executados pelo Cloud Manager durante uma implantação de código regular não são executados durante esse processo.
...
Observe que a API pode não ter sucesso quando um pipeline está em uso, seja uma atualização de AEM ou implantação de cliente, dependendo de qual parte do pipeline de fim a fim está sendo executada no momento. A resposta do erro indicará que a solicitação não foi bem-sucedida, embora não indique o motivo específico.

Pode haver cenários em que uma implantação de código agendada do cliente dependa de variáveis existentes para ter novos valores, o que não seria apropriado com o código atual. Se isso for uma preocupação, é recomendável fazer modificações variáveis de uma maneira aditiva. Para fazer isso, crie novos nomes de variáveis em vez de apenas alterar o valor de variáveis antigas, de modo que o código antigo nunca faça referência ao novo valor. Em seguida, quando a nova versão do cliente parecer estável, é possível optar por remover os valores mais antigos.

Da mesma forma, como os valores de uma variável não têm controle de versão, uma reversão do código pode fazer com que faça referência a valores mais recentes que causem problemas. A estratégia de variável de aditivos acima mencionada também ajudaria neste caso.
