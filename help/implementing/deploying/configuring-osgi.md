...
As alterações de configuração são definidas nos pacotes de código do projeto AEM (`ui.config`) como arquivos de configuração (`.cfg.json`) em pastas de configuração específicas do modo de execução:

`/apps/example/config.<runmode>`

O formato dos arquivos de configuração do OSGi é baseado em JSON usando o `.cfg.json` formato definido pelo projeto Apache Sling.
...
após o `cfg.json` Formato de configuração do OSGi.

>[!NOTE]
>
>Versões anteriores do AEM eram compatíveis com arquivos de configuração OSGi usando diferentes formatos de arquivo, como `.cfg`, `.config` e como XML `sling:OsgiConfig` definições de recursos. Esses formatos são substituídos pelo `.cfg.json` Formato de configuração do OSGi.

>[!NOTE]
>
>As configurações de OSGi não são armazenadas em /apps como instâncias típicas do AEM na nuvem. Elas são armazenadas em um local externo. Fazer check-in no Cloud Manager [Console do desenvolvedor](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/developer-console#configurations) para exibir as configurações de OSGi.

## Resolução do modo de execução {#runmode-resolution}

>[!TIP]
>
>O AEM 6.x é compatível com modos de execução personalizados, no entanto o AEM as a Cloud Service não. Suporte as a Cloud Service para AEM e [conjunto exato de modos de execução](./overview.md#runmodes). Qualquer variação nas configurações de OSGi entre ambientes as a Cloud Service AEM deve ser tratada usando [Variáveis de ambiente de configuração do OSGi](#environment-specific-configuration-values).
...
Por exemplo, se o AEM estiver usando o autor de modos de execução e o desenvolvimento, os nós de configuração no `/apps/example/config.author/` e `/apps/example/config.author.dev/` são aplicados, enquanto os nós de configuração em `/apps/example/config.publish/` e `/apps/example/config.author.stage/` não são aplicadas.
...
>[!NOTE]
>
>A `config.preview` Pasta de configuração do OSGi **não é possível** deve ser declarado da mesma forma que `config.publish` pode ser declarada pasta. Em vez disso, o nível de visualização herda sua configuração OSGi dos valores do nível de publicação.
...
Os modos de execução as a Cloud Service do AEM são bem definidos com base no tipo de ambiente e no serviço. Revise o [lista completa de modos de execução AEM as a Cloud Service disponíveis](./overview.md#runmodes).
...
A visualização resultante exibe todas as configurações do componente OSGi para as camadas selecionadas com seus valores de configuração OSGi aplicáveis. Esses valores podem ser referenciados juntamente com os valores de configuração do OSGi no código-fonte do projeto AEM em `/apps/example/osgiconfig/config.<runmode(s)>`.


Para verificar se os valores de configuração OSGi apropriados foram aplicados:

1. Na saída da Configuração do Console do desenvolvedor
1. Localize o `pid` representando a configuração do OSGi a ser verificada; esse é o nome do arquivo de configuração do OSGi no código-fonte do projeto AEM.
1. INSPECT o `properties` lista para o `pid` e verifique se a chave e os valores correspondem ao arquivo de configuração OSGi no código-fonte do projeto AEM para o modo de execução que está sendo verificado.=
...
1. **Valores em linha**, que são valores codificados na configuração do OSGi e armazenados no Git. Por exemplo:
...
1. **Valores específicos do ambiente**, que são valores que variam entre ambientes de desenvolvimento e, portanto, não podem ser direcionados com precisão pelo modo de execução (já que há uma única `dev` modo de execução no Adobe Experience Manager as a Cloud Service). Por exemplo:
...
1. Cole as Propriedades da configuração serializada da Etapa 10 na `.cfg.json` arquivo.
1. Salve as alterações no novo `.cfg.json` arquivo.
1. Adicione e confirme o novo arquivo de configuração OSGi no Git.
...
>[!NOTE]
>
>Os marcadores de posição não podem ser usados em [instruções repoinit](/help/implementing/deploying/overview.md#repoinit).
...
Por exemplo, se `$[secret:server_password]` for usado, um arquivo de texto chamado **server_password** deve ser criado. Todos esses arquivos secretos devem ser armazenados no mesmo diretório e na propriedade da estrutura `org.apache.felix.configadmin.plugin.interpolation.secretsdir` deve ser configurado com esse diretório local.
...
A intenção é para o valor da propriedade OSGi `my_var1` ser o mesmo para estágio e produção, mas diferentes para cada um dos três ambientes de desenvolvimento.
...
A intenção é para o valor da propriedade OSGi `my_var1` para diferir para estágio, produção e para cada um dos três ambientes de desenvolvimento. Portanto, a API do Cloud Manager deve ser chamada para definir o valor de `my_var1` para cada ambiente dev.
...
A intenção é para o valor da propriedade OSGi `my_var1` ser o mesmo para ambientes de preparo, produção e apenas um dos ambientes dev, mas para diferir para os outros dois ambientes dev. Nesse caso, a API do Cloud Manager deve ser chamada para definir o valor de `my_var1` para cada um dos ambientes dev, incluindo para o ambiente dev, que deve ter o mesmo valor do estágio e da produção. Ele não herdará o valor definido na pasta **config**.
...
Outra maneira de fazer isso seria definir um valor padrão para o token de substituição na pasta config.dev de modo que seja o mesmo valor que no **config** pasta.
